...
//	if($db->get_row($sql)) sys_msg("此案例信息今天已添加！", 1);
	...
	$link[0]['href'] = 'trends.php?act=add';
	$link[1]['text'] = '返回案例列表';
	$link[1]['href'] = 'trends.php?act=list';
...
if ($_REQUEST['act'] == 'list')
{
	...
	*/
	...
	$smarty->assign('action_href',  "trends.php?act=add");
	...
	$smarty->display('trends_list.html');
}
...
//	if($db->get_row($sql)) sys_msg("此案例信息今天已添加！", 1);
	...
	$link[0]['href'] = 'trends.php?act=list';
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'trends.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
	...
	if ($result)
	{
		...
		$link[] = array('text' => '返回交易信息列表', 'href' => 'trends.php?act=list');
...
{	
	...
	if ($result)
	{
		...
		$link[] = array('text' => '返回列表', 'href' => 'trends.php?act=list');
