...
class cls_sql_dump
{
    ...
    function get_table_df($table, $add_drop = false)
    {
        ...
        $tmp_arr = $this->db->get_one("SHOW CREATE TABLE `$table`");
        $tmp_sql = $tmp_arr['Create Table'];
        $tmp_sql = substr($tmp_sql, 0, strrpos($tmp_sql, ")") + 1); //去除行尾定义。

        if ($this->db->version() >= '4.1')
        {
           $table_df .= $tmp_sql . " ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=2;\r\n ";
    ...
    function get_table_data($table, $pos)
    {
        ...
        /* 获取数据表记录总数 */
        $total = $this->db->get_row("SELECT COUNT(*) FROM $table");
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END lk v1.0 SQL Dump Program ';
    ...
    function make_head($vol)
    {
        /* 系统信息 */
        $sys_info['os']         = PHP_OS;
        $sys_info['web_server'] = $GLOBALS['lk']->get_domain();
        $sys_info['php_ver']    = PHP_VERSION;
        $sys_info['mysql_ver']  = $this->db->version();
        $sys_info['date']       = date('Y-m-d H:i:s');

        $head = "-- lk v1.0 SQL Dump Program\r\n".
                 "-- " . $sys_info['web_server'] . "\r\n".
                 "-- \r\n".
                 "-- DATE : ".$sys_info["date"]."\r\n".
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'lk_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'lk VERSION')
                {
                    $sql_info['lk_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
