...
$exc = new exchange($lk->table("case_trends"), $db, 'Case_id', 'title');
...
if ($_REQUEST['act'] == 'add')
{
	...
	$smarty->assign('action_href',  "trends2.php?act=list");
	$smarty->assign('ur_here',      "添加案例信息");
	
	$smarty->assign('form_act',     'insert');
	$smarty->display('trends_info.html');
}
if ($_REQUEST['act'] == 'insert')
{
	...
	$cat_id = 13;   //日先动态的cat_id是13
	$addtime = time();
	
//	/* 标题是否相同 */
// 	$sql = "SELECT COUNT(*) FROM " . $lk->table('case_trends') . " 
...
//	if($db->get_row($sql)) sys_msg("此案例信息今天已添加！", 1);
	...
	$set = "title = '".$_POST['title']."',
			...
			cat_id  = '".$cat_id."'";
	//var_dump($set);exit;//test
	$exc->insert($set);

	/*添加链接*/
	$link[0]['text'] = '继续添加案例信息';
	$link[0]['href'] = 'trends2.php?act=add';
	$link[1]['text'] = '返回案例列表';
	$link[1]['href'] = 'trends2.php?act=list';
...
if ($_REQUEST['act'] == 'list')
{
	...
	*/
	...
	$smarty->assign('action_href',  "trends2.php?act=add");
	...
	$smarty->display('trends2_list.html');
}
elseif ($_REQUEST['act'] == 'query')    //查询之后调用的是这里的模板
{
    $Case = get_Case();
    ...
    make_json_result($smarty->fetch('trends_list.html'), '',
...
if ($_REQUEST['act'] == 'edit')
{
	...
	$smarty->display('trends2_info.html');
}
...
//	if($db->get_row($sql)) sys_msg("此案例信息今天已添加！", 1);
	...
	$link[0]['href'] = 'trends2.php?act=list';
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'trends2.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
	...
	if ($result)
	{
		...
		$link[] = array('text' => '返回交易信息列表', 'href' => 'trends2.php?act=list');
...
{	
	...
	if ($result)
	{
		...
		$link[] = array('text' => '返回列表', 'href' => 'trends2.php?act=list');
		sys_msg(sprintf('成功设置 %d 篇资讯', $count), 0, $link);
	}else{
		$link[] = array('text' => '返回列表', 'href' => 'trends2.php?act=list');
		sys_msg('您没有选择要删除的记录!', 1, $link);	
	}
...
{	
	...
	if ($result)
	{
		...
		$link[] = array('text' => '返回列表', 'href' => 'trends2.php?act=list');
		sys_msg(sprintf('成功取消了 %d 篇资讯', $count), 0, $link);
	}else{
		$link[] = array('text' => '返回列表', 'href' => 'trends2.php?act=list');
...
function get_Case()
{
	...
	*/
    ...
    /* 获得总记录数据 */
  	$sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['lk']->table('case_trends'). ' AS d ' . $where ;
    ...
    $list = array();
	$sql = "SELECT d.*, c.cat_name FROM " .$GLOBALS['lk']->table('case_trends'). " AS d 
