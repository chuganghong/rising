...
$exc = new exchange($lk->table("brand_album"), $db, 'cat_id', 'cat_name');
$Caseloca = new category($lk->table("brand_album"));
...
if ($_REQUEST['act'] == 'add')
{
	...
	*/
	...
	$smarty->assign('action_href',  "album.php?act=list");
	$smarty->assign('ur_here',      "添加案例栏目");
	
	$smarty->assign('form_act',     'insert');
	
	$smarty->display('album_info.html');
}
...
if ($_REQUEST['act'] == 'insert')
{
	...
	*/
	...
	//新建album时，在table sd_brand中insert 5条cat_id为mysql_insert_id()的数据
	$cat_id2 = mysql_insert_id();
	//var_dump($cat_id2);exit;
	$exc2 = new exchange($lk->table("brand"), $db, 'flash_id', 'img_title');
	...
	$link[0]['href'] = 'album.php?act=list';
	
	$link[1]['text'] = '继续添加案例栏目';
	$link[1]['href'] = 'album.php?act=add&Caselocaid='.$selMenu;
...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_text',  "添加图集");
	$smarty->assign('action_href',  "album.php?act=add");
	$smarty->assign('ur_here',      "图集列表");
	$smarty->display('album_list.html');
}
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 当前分类下是否有子信息 */
    $Case_count = $db->get_row("SELECT COUNT(*) FROM " .$lk->table('brand'). " WHERE cat_id = '$cat_id'");	
    ...
    $url = 'album.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('album_list.html'));
}
?>